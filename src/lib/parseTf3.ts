...
export function parseTf3(buffer: ArrayBuffer): Tf3ParseResult {
  if (buffer.byteLength < 16) {
    throw new Error("TF3ファイルが短すぎるため解析できません。");
  }
  ...
  const arrivalDateTime = primaryDateTime.value ?? secondaryDateTime.value;
  const asciiCandidate = decodeAsciiDigits(readBytes(view, 0x0002, 6));

  if (!arrivalDateTime) {
    warnings.push("TF3から入庫時刻候補を取得できませんでした。");
  }

  return {
    serviceDateCandidate: dateFromDateTime(arrivalDateTime),
    arrivalTime: timeFromDateTime(arrivalDateTime),
    arrivalDateTime,
